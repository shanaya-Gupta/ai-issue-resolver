...
SEARCH_QUERY = 'is:issue is:open label:"good first issue"' # Language agnostic
...
def find_github_issues():
    ...
    try:
        ...
        for issue in items:
            ...
            if issue.get('comments', 0) > 10: continue # Skip long discussions
            body = issue.get('body') or ""
            if len(body) < 100: continue # Skip very short descriptions
...
def get_repo_context(temp_dir: str):
    ...
    try:
        ...
        for root, dirs, files in os.walk(temp_dir):
            ...
            if len(structure) > 300: break # Limit total lines
        context["structure"] = '\n'.join(structure)
        ...
        for root, _, files in os.walk(temp_dir):
            ...
            for pf in priority_files:
                if pf in files:
                    files.insert(0, files.pop(files.index(pf)))
            for file in files:
                if file.endswith(extensions):
                    ...
                    try:
                        ...
                        if total_size > 500000: return context # Stop if context gets too large
                    except Exception: pass
    except Exception as e:
        print(f"⚠️ Error gathering repo context: {e}")
    return context

def validate_code_syntax(file_path: str, code: str) -> bool:
    """Basic syntax validation. Returns True if valid."""
    if not code.strip(): return False
    ext = Path(file_path).suffix.lower()
    if ext == '.py':
        try: compile(code, file_path, 'exec'); return True
        except SyntaxError: return False
    if ext == '.json':
        try: json.loads(code); return True
        except json.JSONDecodeError: return False
    return True # Basic check for other languages
...
def process_issue(issue):
    ...
    try:
        ...
        print(f"❌ Git setup failed: {e}"); return
    ...
    3. Create a brief, step-by-step strategy for the fix.
    ...
    try:
        ...
        if file_to_change == "N/A":
            print("🤷 Planner could not identify a relevant file. Skipping."); return
    except Exception as e:
        print(f"❌ Planner failed: {e}"); return
        
    # --- AGENT 2: THE IMPLEMENTER (FLASH) ---
    print(f"\n--- ⚙️  Stage 2: Implementation for {file_to_change} (Gemini Flash) ---")
    original_file_content = repo_context['files'].get(file_to_change)
    if original_file_content is None:
        print(f"❌ Critical Error: Planner identified file '{file_to_change}' but it was not found in context."); return
    ...
    Review for correctness, style, and robustness. Think about side effects.
    ...
    # --- VALIDATION & APPLY ---
    if not validate_code_syntax(file_to_change, final_code):
        print("❌ Validation failed: Critic produced invalid code. Aborting."); return
    print(f"✅ Code validated. Applying fix to {file_to_change}")
    full_path_to_file = Path(temp_dir) / file_to_change
    try:
        full_path_to_file.parent.mkdir(parents=True, exist_ok=True)
        full_path_to_file.write_text(final_code, encoding='utf-8')
        print("💾 File overwritten successfully.")
    except Exception as e:
        print(f"❌ Failed to write file: {e}"); return
    ...
    try:
        ...
        print(f"❌ Git push failed: {e}"); return
    ...
    pr_data = { 'title': f"[AI] Fix: {issue['title']}", 'body': pr_body, 'head': f"{GITHUB_USERNAME}:{new_branch}", 'base': base_branch }
